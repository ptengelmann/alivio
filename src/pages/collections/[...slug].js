      ...
      <>
        ...
        <div className="min-h-screen bg-[#0a0a0f] text-white flex items-center justify-center pt-20">
          <div className="text-center px-6">
            <h1 className="text-4xl lg:text-5xl font-light text-white mb-6 tracking-tight">Collection Not Found</h1>
            <p className="text-zinc-500 text-xs tracking-wide mb-8 font-light">The requested collection is not in our database.</p>
            <Link href="/collections" className="inline-block border border-white text-white py-3 px-10 text-[10px] uppercase tracking-[0.2em] font-light hover:bg-white hover:text-black transition-all duration-300" data-cursor="BACK">
              Return to Shop
            </Link>
          </div>
        </div>
      ...
      <main className="min-h-screen bg-[#0a0a0f] text-white pt-20">
        ...
        <div className="py-6 border-b border-zinc-900/50">
          <div className="max-w-[1600px] mx-auto px-8 lg:px-12 flex justify-between items-center">
            <Link
              href="/collections"
              className="inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] text-zinc-500 hover:text-white transition-colors font-light"
              data-cursor="BACK"
            >
          ...
          <div className="max-w-[1600px] mx-auto px-8 lg:px-12">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
              <div>
                ...
                <h1 className="text-5xl lg:text-6xl font-light text-white mb-8 leading-tight tracking-tight uppercase">
                  {emotion?.name || collection.title}
                </h1>

                <div className="flex items-center gap-6 text-[10px] text-zinc-600 mb-10 flex-wrap uppercase tracking-wider">
              ...
              {emotion?.contraband && (
                <div className="border border-zinc-700/30 p-8 lg:p-10 bg-zinc-900/20">
                  <div className="flex items-center gap-3 mb-8">
                    <Database className="w-4 h-4 text-zinc-600" />
                    <span className="text-[9px] tracking-[0.3em] text-zinc-600 uppercase">Classification_Data</span>
                  </div>

                  <div className="space-y-1">
                    {emotion.contraband.batchNumber && (
                      <div className="flex justify-between py-3 border-b border-zinc-700/30">
                        <span className="text-[10px] text-zinc-500 uppercase tracking-wider font-light">Batch Number</span>
                        <span className="text-[10px] text-white tracking-wider">{emotion.contraband.batchNumber}</span>
                      </div>
                    )}
                    {emotion.contraband.purity && (
                      <div className="flex justify-between py-3 border-b border-zinc-700/30">
                        <span className="text-[10px] text-zinc-500 uppercase tracking-wider font-light">Purity</span>
                        <span className="text-[10px] text-white tracking-wider">{emotion.contraband.purity}</span>
                      </div>
                    )}
                    {emotion.contraband.potency && (
                      <div className="flex justify-between py-3 border-b border-zinc-700/30">
                        <span className="text-[10px] text-zinc-500 uppercase tracking-wider font-light">Potency</span>
                        <span className="text-[10px] tracking-wider" style={{ color: emotion.colors.accent }}>{emotion.contraband.potency}</span>
                      </div>
                    )}
                    <div className="flex justify-between py-3">
                      <span className="text-[10px] text-zinc-500 uppercase tracking-wider font-light">Status</span>
                      <span className="text-[10px] text-green-500/70 uppercase tracking-wider">Verified</span>
                    </div>
                  </div>

                  {emotion.contraband?.symptoms && emotion.contraband.symptoms.length > 0 && (
                    <div className="mt-8 pt-8 border-t border-zinc-700/30">
                      <div className="text-[9px] tracking-[0.3em] text-zinc-500 mb-4 uppercase">Symptoms</div>
          ...
          <div className="max-w-[1600px] mx-auto px-8 lg:px-12">
            {products.length === 0 ? (
              <div className="text-center py-24">
                <div className="text-zinc-600 text-xs uppercase tracking-wider font-light mb-2">No Items In Inventory</div>
                <div className="text-zinc-700 text-[10px] tracking-wide font-light">Check back soon for new releases</div>
              </div>
            ) : (
              <>
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          {/* Product Info */}
                          <div className="space-y-1">
                            <h3 className="text-xs font-light text-white leading-tight tracking-wide uppercase">
                              {product.title}
                            </h3>
